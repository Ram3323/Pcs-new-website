...
//   const [errors, setErrors] = useState({});
//   const [apiResponse, setApiResponse] = useState(null); // For API success/failure message
//   const leftSectionRef = useRef(null);
//   const rightSectionRef = useRef(null);
...
//             phone_number: "1234567890", // You can make this dynamic if needed
...
//         if (response.ok) {
//           setApiResponse("Message sent successfully!");
//         } else {
...
//               <p className={apiResponse.includes('success') ? 'text-green-500' : 'text-red-500'}>
//                 {apiResponse}
//               </p>
//             </div>
//           )}
      ...
      <div className="flex flex-col lg:flex-row mx-4 md:mx-24 shadow-lg w-full">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-10">
            ...
            <div className="relative">
              <TbMessageChatbot className="absolute left-5 top-1/2 transform h-[15px] w-[15px] -translate-y-1/2 text-gray-400" />
              <textarea
                name="message"
                value={formData.message}
                onChange={handleChange}
                placeholder="MESSAGE"
                rows={4}
                className={`pl-12 pr-4 py-3 w-full text-[12px] rounded-3xl ${errors.message ? 'border-red-500' : ''}`}
              ></textarea>
              {errors.message && <p className="text-red-500 text-xs mt-1">{errors.message}</p>}
            </div>

            <div className="flex justify-center">
              <button type="submit" className="bg-blue-500 text-white px-8 py-3 rounded-full text-sm">Send Message</button>
            </div>
